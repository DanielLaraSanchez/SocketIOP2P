class Moderator {
    ...
    checkIfPeerIsInSpecificConnection(connArray, peerId){
        ...
        for(let i = 0; i < connArray.length; i++){
            if(connArray[i].reciever === peerId || connArray[i].sender === peerId){
                resultBoolean = true;
            }else{
                resultBoolean = false;
            }
    ...
    pairUpAfterDisconnection(connectionArray) {
        let arrayOfUnparedConnections = connectionArray.filter((connection) => {
            return (connection.sender === null && connection.reciever != null) || (connection.reciever === null && connection.sender != null)
        });
        ...
        while(arrayOfUnparedConnections.length >= 2){
           ...
           let unpairedConn2 = arrayOfUnparedConnections[1];
            connectionArray.forEach((connection) => {
                if(connection.id === unpairedConn1.id && connection.reciever === null){
                ...
                }else if(connection.id === unpairedConn1.id && connection.sender === null){
                    let movingPeer = this.returnUnpairedPeerIdInConn(unpairedConn2);
